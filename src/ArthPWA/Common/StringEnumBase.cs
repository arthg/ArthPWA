...
namespace ArthPWA.Common
{
    ...
    /// </code>
    public abstract class StringEnumBase : IComparable<StringEnumBase>, ISerializable
    {
        ...
        private const string SERIALIZATION_NAME = "Name";

        protected StringEnumBase(SerializationInfo serializationInfo, StreamingContext streamingContext) : this()
        {
            var fieldName = serializationInfo.GetString(SERIALIZATION_NAME);
            _value = GetFieldDataForType(this.GetType()).First(f=>f.FieldName == fieldName).StringValue;
        }

        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            var fieldData = GetFieldDataForType(this.GetType());
            var fieldInfo = fieldData.First(f => f.StringValue == _value);
            info.AddValue(SERIALIZATION_NAME, fieldInfo.FieldName);
        }
        #endregion
